---
title: "Bulk RNAseq report - Differential Expression Analysis among conditions"
author: 
- Annamaria Piva
...
## Introduction
The goal of this analysis is to identify which pathways are up- or down-regulated in each condition (Healthy, HR+, and TNBC).
...
load(paste0(folderpath, 'data/dds.RData')) # dds (condition)
...
datatable(
  sample_info[, c("samplename",  "condition" ,  "aneuploidy_score",  "ENSG00000085117", "X.POS", "INTENSITA." , "TOTAL.SCORE.1." ,"CATEGORY.1." ,"IRS.2.","IRS.classification.2.", "NOTE", "Controparte.tumorale", "batch" )],
  extensions = 'Buttons',
  options = list(
    dom = 'Blfrtip',
...
mydeseq2 = function(dds, min_reads, sample_info, min_sample = 5, ...){
  ...
  ## Normalize the data using vst (for large dataset), and not rlog (with batch effect considered)
...
The batch effect has been considered in the design.
```{r pca}
...
Quantifies the overall dissimilarity in expression profiles between samples, based on their absolute expression values. Smaller distances indicate more similar samples.
...
```{r sample_corr, fig.width=8, fig.height=8}

vsd <- vst(dds, blind=FALSE)
sample_dists <- dist(t(assay(vsd)))
sample_cor <- cor(assay(vsd), method = "pearson")  # or method = "spearman"


# Convert distances to matrix form
sample_dist_matrix <- as.matrix(sample_dists)

# add sample names as row and colnames
...
annotation_df <- as.data.frame(sample_info[, c("condition", "batch"), drop = FALSE])
colnames(annotation_df) <- c("Condition", "Batch")
...
pheatmap(sample_cor, 
         # clustering_distance_rows = sample_cor,
         # clustering_distance_cols = sample_cor, 
...
```{r sample_dist, fig.width=8, fig.height=8}
...
contrasts=c("HRplus_vs_HLT" , "TNBC_vs_HLT" , "TNBC_vs_HRplus") #  unique(all$comparison_n_vs_d) # "HRplus_vs_HLT" , "TNBC_vs_HLT" , "TNBC_vs_HRplus"

contrast = "HRplus_vs_HLT"
...
plotly::plot_ly(data=tmp, x=~PC1, y=~PC2, text = ~name, color=~condition, colors=annotation_colors$Condition ) %>%
  layout(
    xaxis = list(title = paste0("PC1 (", round(percentVar[1] * 100, 1), "% variance)")),
    yaxis = list(title = paste0("PC2 (", round(percentVar[2] * 100, 1), "% variance)"))
  )
...
```{r volcano_plot1}
plotly::plot_ly(data=res, x=~log2FoldChange, y=~-log10(padj), color=~significant, text = ~gene, title="Volcano plot")  %>% layout(title = sprintf("Volcano plot - %s", contrast)) %>% add_annotations( x = -15, y = 50, text = paste0("n. down-reg:\n", table(res[res$significant=="significant","deg"])['down-reg'] ), showarrow = FALSE,font = list(family = "Arial", size = 16, color = "blue"),xanchor = 'left', yanchor = 'middle') %>% add_annotations( x = 15, y = 50, text = paste0("n. up-reg:\n", table(res[res$significant=="significant","deg"])['up-reg'] ), showarrow = FALSE,font = list(family = "Arial", size = 16, color = "red2"),xanchor = 'left', yanchor = 'middle')
```
...
Given the significant genes, among the differentially expressed genes previously computed, let's visualize the top20.
...
data_pos_vs_neg <- assay(rld)[top20_genes$gene, ]
pheatmap(
...
<!-- all_data_pos_vs_neg <- assay(rld)[de_genes,]  -->

<!-- pheatmap( t(scale(t(all_data_pos_vs_neg))), cluster_rows = FALSE, cluster_cols = FALSE,  -->
<!--           show_rownames = TRUE, show_colnames = TRUE, color = colorRampPalette(c("blue", "white", "red"))(50),  -->
...
knitr::include_graphics(paste0(folderpath, "analysis/hsa04064.HRplus_vs_HLT.png"))
...
venn.plot <- venn.diagram(
  x = list( HRplus_vs_HLT = paths1,  TNBC_vs_HLT = paths2, TNBC_vs_HRplus = paths3),
  filename = NULL,
...
# pdf(sprintf( '%s/output/gse_results/GSEA_heatmap.pdf', folderpath) )
...
```{r all_genes_table}
datatable(
  all[, c("gene","gene_symbol","comparison_n_vs_d", "log2FoldChange", "padj" , "differentially_expressed"  )],
  extensions = 'Buttons',
  options = list(
    dom = 'Blfrtip',
    buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
...
write.table(x=all, file=paste0(folderpath, "output/all_degs_condition.csv"), quote=F, row.names=F, sep=",")
