---
title: "Bulk RNAseq report - Differential Expression Analysis based on CD82 Gene Expression Levels"
author: 
- Annamaria Piva
...
# Introduction
The goal of this analysis is to identify which pathways are up- or down-regulated in samples with high or low levels of CD82 expression.
For each condition (Healthy, HR+, and TNBC), patients are divided into high and low CD82 gene expression groups using the median expression value as a cutoff. 
...
sample_info$CD82_group <- NA

# for (cond in unique(sample_info$condition)) {
#   samples_in_cond <- sample_info$condition == cond
#   scores <- sample_info$ENSG00000085117[samples_in_cond]
#   median_score <- median(scores, na.rm = TRUE)
#   
#   sample_info$CD82_group[samples_in_cond] <- ifelse(
#     scores > median_score, "HighCD82", "LowCD82"
#   )
# }

median_score= median(sample_info$ENSG00000085117, na.rm = TRUE)
sample_info$CD82_group <- ifelse(
    sample_info$ENSG00000085117 > median_score, "HighCD82", "LowCD82"
  )

sample_info$CD82_group <- factor(sample_info$CD82_group,
                                               levels = c("LowCD82", "HighCD82"))
sample_info$CD82_group_by_condition <- paste0(sample_info$condition, "-", sample_info$CD82_group)
...
datatable(
  sample_info[, c("samplename", "condition" , "aneuploidy_score", "ENSG00000085117", "CD82_group", "X.POS", "INTENSITA.", "TOTAL.SCORE.1.", "CATEGORY.1." ,"IRS.2.","IRS.classification.2.", "NOTE", "Controparte.tumorale", "batch" )],
  extensions = 'Buttons',
  options = list(
    dom = 'Blfrtip',
...
plot_hist_with_median <- function(df, group_label) {
  hist(df$ENSG00000085117,
       breaks = 50,
       main = paste("CD82 expression Distribution -", group_label),
       xlab = "CD82 expression",
       ylab = "Frequency",
       col = "lightgray",
       border = "white", 
       prob=TRUE)
  
  abline(v = median(df$ENSG00000085117, na.rm = TRUE),
         col = "blue", lwd = 2, lty = 2)
  
    abline(v = median(sample_info$ENSG00000085117, na.rm = TRUE),
         col = "red", lwd = 2, lty = 2)
}
...
ggplot(sample_info, aes(x = CATEGORY.1., y = ENSG00000085117, fill=CD82_group_by_condition)) +
  geom_violin(alpha = 0.7, outlier.shape = NA) +
  geom_jitter(width = 0.2, size = 1, alpha = 0.5) +
  labs(
    title = "CD82 expression by Condition and Categories defined by IHC",
    x = "Condition",
    y = "CD82 expression",
    fill = "CD82_group"
  ) +
...
ggplot(sample_info, aes(x = CD82_group_by_condition, y = ENSG00000085117, fill = CD82_group_by_condition)) +
  geom_boxplot(alpha = 0.7, outlier.shape = NA) +
  geom_jitter(width = 0.2, size = 1, alpha = 0.5) +
  labs(
    title = "CD82 expression by Condition and High/Low Group based on median expression levels",
    x = "Condition",
    y = "CD82 expression",
    fill = "CD82_group"
  ) +
...
sample_info$condition=sample_info$CD82_group_by_condition
...
# Extract the % variance explained from attributes of tmp
...
contrasts=c("HRplus-HighCD82_vs_HRplus-LowCD82", "TNBC-HighCD82_vs_TNBC-LowCD82", "HLT-HighCD82_vs_HLT-LowCD82")
contrast = contrasts[1]

# Define the conditions to compare
selected_conditions <- unlist(strsplit(contrast, "_vs_"))

# Subset, transform, and plot PCA
tmp =plotPCA(
  rld[, rld$condition %in% selected_conditions] |> 
        {\(x) { x$condition <- droplevels(x$condition); x }}(),
  intgroup = "condition", returnData=TRUE
)

percentVar <- attr(tmp, "percentVar")

plotly::plot_ly(data=tmp, x=~PC1, y=~PC2, text = ~name, color=~condition, colors=c("red", "royalblue") ) %>%
  layout(
    xaxis = list(title = paste0("PC1 (", round(percentVar[1] * 100, 1), "% variance)")),
    yaxis = list(title = paste0("PC2 (", round(percentVar[2] * 100, 1), "% variance)"))
  )
...
```{r volcano_plot1}
plotly::plot_ly(data=res, x=~log2FoldChange, y=~-log10(padj), color=~significant, text = ~gene, title="Volcano plot")  %>% layout(title = sprintf("Volcano plot - %s", contrast)) %>% add_annotations( x = -20, y = 40, text = paste0("n. down-reg:\n", table(res[res$significant=="significant","deg"])['down-reg'] ), showarrow = FALSE,font = list(family = "Arial", size = 16, color = "blue"),xanchor = 'left', yanchor = 'middle') %>% add_annotations( x = 20, y = 40, text = paste0("n. up-reg:\n", table(res[res$significant=="significant","deg"])['up-reg'] ), showarrow = FALSE,font = list(family = "Arial", size = 16, color = "red2"),xanchor = 'left', yanchor = 'middle')
```
...
top20_genes <- final_table %>%
  as.data.frame() %>%
  filter(!is.na(gene_symbol)  & trimws(gene_symbol) != "") %>%  ## to have filter gene symbol
  arrange(padj) %>%
  head(20) 

# "HRplus-HighCD82_vs_HRplus-LowCD82", "TNBC-HighCD82_vs_TNBC-LowCD82", "HLT-HighCD82_vs_HLT-LowCD82"
...
df_colors <- list(
  Type = c(    "HLT" = "#66c2a5",
    ...
    "TNBC" = "#8da0cb"), 
  Condition = c(
    "HRplus-HighCD82" = "orange",
    "HRplus-LowCD82" = "tomato4", 
   "HLT-HighCD82" ='lightgreen',   
    "HLT-LowCD82"='darkgreen',
   "HLT-NA"='grey',
         "TNBC-LowCD82" ='lightblue',
         "TNBC-HighCD82" ='blue'
...
top20_counts <- all[top20_genes$gene, ]
data_pos_vs_neg <- assay(rld)[top20_genes$gene, rownames(sample_info[ sample_info$type=='HRplus',]) ]
pheatmap(
  t(scale(t(data_pos_vs_neg))),
  labels_row = top20_genes$gene_symbol,
  cluster_rows = FALSE,
  cluster_cols = TRUE,
  show_rownames = TRUE,
  show_colnames = TRUE,
  color = colorRampPalette(c("blue", "white", "red"))(100),
  breaks =  seq( -4, 4, length.out= 101), # seq( -max(abs(data_pos_vs_neg)), max(abs(data_pos_vs_neg)), length.out= 101),
...
de_counts <- all[de_genes$gene, ]
all_data_pos_vs_neg <- assay(rld)[de_genes$gene, rownames(sample_info[ sample_info$type=='HRplus',]) ]
pheatmap(
  t(scale(t(all_data_pos_vs_neg))),
  labels_row = de_genes$gene_symbol,
  cluster_rows = TRUE,
  cluster_cols = TRUE,
  show_rownames = TRUE,
  show_colnames = TRUE,
...
```{r gene_set_enrichment_analysisBP1}
...
datatable(
  ...
  rownames = FALSE,
  caption = paste0("BP - ", contrast)
)
```

### Cellular Components (CC)
```{r gene_set_enrichment_analysisCC1}
gse <- gseGO(geneList=sort(genelist, decreasing = T), 
                 ont = "CC", 
                 keyType = "ENSEMBL", 
                 minGSSize = 3, 
...
datatable(
  ...
  rownames = FALSE,
  caption = paste0("CC - ", contrast)
)
```

### Molecular function (MF)
```{r gene_set_enrichment_analysisMF1}
gse <- gseGO(geneList=sort(genelist, decreasing = T), 
                 ont = "MF", 
                 keyType = "ENSEMBL", 
                 minGSSize = 3, 
...
datatable(
  ...
  rownames = FALSE,
  caption = paste0("MF - ", contrast)
)
```


### GSEA with Hallmark Pathways (msigdbr)
To identify biologically meaningful patterns of gene expression, we performed Gene Set Enrichment Analysis (GSEA) using the MSigDB Hallmark gene sets, which summarize well-defined biological states or processes.
Genes were ranked by Log2 Fold Change. 
Significantly enriched pathways were identified based on normalized enrichment score (NES) and adjusted p-values (FDR) (p.adj < 0.05). Positively enriched pathways are upregulated in the first group, while negatively enriched pathways indicate suppression.
...
top20_counts <- all[top20_genes$gene, ]
data_pos_vs_neg <- assay(rld)[top20_genes$gene, rownames(sample_info[ sample_info$type=='TNBC',]) ]
pheatmap(
  t(scale(t(data_pos_vs_neg))),
  labels_row = top20_genes$gene_symbol,
...
de_counts <- all[de_genes$gene, ]
all_data_pos_vs_neg <- assay(rld)[de_genes$gene, rownames(sample_info[ sample_info$type=='TNBC',]) ]
pheatmap(
  t(scale(t(all_data_pos_vs_neg))),
    labels_row = de_genes$gene_symbol,
...
### Biological Processes (BP)
```{r gene_set_enrichment_analysisBP2}
...
### Cellular Components (CC)
```{r gene_set_enrichment_analysisCC2}
gse <- gseGO(geneList=sort(genelist, decreasing = T), 
                 ont = "CC", 
                 keyType = "ENSEMBL", 
                 minGSSize = 3, 
...
### Molecular function (MF)
```{r gene_set_enrichment_analysisMF2}
gse <- gseGO(geneList=sort(genelist, decreasing = T), 
                 ont = "MF", 
                 keyType = "ENSEMBL", 
                 minGSSize = 3, 
...
top20_counts <- all[top20_genes$gene, ]
data_pos_vs_neg <- assay(rld)[top20_genes$gene, rownames(sample_info[ sample_info$type=='HLT',]) ]
pheatmap(
  t(scale(t(data_pos_vs_neg))),
  labels_row = top20_genes$gene_symbol,
  cluster_rows = FALSE,
...
de_counts <- all[de_genes$gene, ]
all_data_pos_vs_neg <- assay(rld)[de_genes$gene, rownames(sample_info[ sample_info$type=='HLT',]) ]
pheatmap(
  t(scale(t(all_data_pos_vs_neg))),
  labels_row = de_genes$gene_symbol,
...
### Biological Processes (BP)
```{r gene_set_enrichment_analysisBP3}
...
### Cellular Components (CC)
```{r gene_set_enrichment_analysisCC3}
gse <- gseGO(geneList=sort(genelist, decreasing = T), 
                 ont = "CC", 
                 keyType = "ENSEMBL", 
                 minGSSize = 3, 
...
### Molecular function (MF)
```{r gene_set_enrichment_analysisMF3}
gse <- gseGO(geneList=sort(genelist, decreasing = T), 
                 ont = "MF", 
                 keyType = "ENSEMBL", 
                 minGSSize = 3, 
...
datatable(
  res1[res1$ID %in% common_paths, c('ID', 'Description')],
  extensions = 'Buttons',
  options = list(
    dom = 'Blfrtip',
    buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
  ...
  rownames = FALSE,
  caption = 'Table of common pathways (BP)'
)
...
datatable(
  res2[res2$ID %in% common_paths, c('ID', 'Description')],
  extensions = 'Buttons',
  options = list(
    dom = 'Blfrtip',
    buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
  ...
  rownames = FALSE,
  caption = 'Table of common pathways (CC)'
)
...
datatable(
  ...
  rownames = FALSE,
  caption = 'Table of common pathways (MF)'
)
...
# pdf(sprintf( '%s/output/gse_results/GSEA_heatmap_cd82levels.pdf', folderpath) )
...
write.table(x=all[(all$comparison_n_vs_d %in% contrasts) ,], file=paste0(folderpath, "/output/all_degs_cd82levels.csv"), quote=F, row.names=F, sep=",")
